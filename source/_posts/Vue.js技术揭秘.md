...
# 准备工作
## 认识flow
[官方地址](https://flow.org/en/docs/types/)
Vue 通过目录下的·`flowconfig`文件中的`[libs]`描述了自定义类型的位置
```
[libs]
flow
```
...
---
## Vue.js 源码构建
Vue.js 源码基于 [Rollup](https://github.com/rollup/rollup) 构建,构建相关代码在 scripts 目录下。
...
```
通过配置我们可以知道入口文件为``src/platforms/web/entry-runtime-with-compiler.js``
...
---
## 从入口开始
Runtime + Complier构建 Vue，入口为 `src/platforms/web/entry-runtime-with-compiler.js`
```js
/* @flow */
...
export default Vue
```
关键代码是`import Vue from 'core/index'`这一行，其他的都是一些对 Vue 的拓展，看一下 `src/core/index.js`，这里才是真正初始化的地方
```js
...
export default Vue
```
这个文件中两处代码比较重要`import Vue from './instance/index'`和`initGlobalAPI(Vue)`,先看一下 `src/core/instance/index.js`
...
```
Vue 官网中介绍的全局 API 都可以在这里找到，需要注意的是 `Vue.util` 暴露的方法最好不要依赖，可能经常会发生变化。
...
## new Vue 发生了什么
Vue 的类在 `src/core/instance/index.js` 中
```js
function Vue (options) {
  if (process.env.NODE_ENV !== 'production' &&
  // 此处判断是否通过new调用
...
## Vue 实例挂载的实现
Vue 通过`$mount`实例方法挂载`vm`，由于和平台构建方式有关，所以`$mount` 在多个文件中都有定义,包括`src/platform/web/entry-runtime-with-compiler.js`、`src/platform/web/runtime/index.js`、`src/platform/weex/runtime/index.js`
我们重点看`compiler`版本的实现
...
Vue.prototype.$mount = function (
  ...
  hydrating?: boolean
    ...
    // 根据传入的内容决定挂载节点，如果是string就通过document.querySelector获取，如果取不到就新建一个div，如果传入一个dom节点就直接使用
  el = el && query(el)
...
`src/core/instance/lifecycle.js`中的`mountComponent`方法
```js
export function mountComponent (
  ...
  // $vnode表示Vue实例的父虚拟Node，如果是null的话表示为根Vue实例
  if (vm.$vnode == null) {
      // 标记实例已经挂载
    vm._isMounted = true
    callHook(vm, 'mounted')
...
## render
`_render`是实例的一个私有方法，用于吧实例渲染为虚拟 Node
...
```
`render`函数，第一个参数是`createElement`
```js
<div id="app">
  {{ message }}
</div>
...
```
`_render`函数中调用了`render`方法
```js
vnode = render.call(vm._renderProxy, vm.$createElement)
```
`render`函数的`createElement`方法就是`vm.$createElement`方法
```js
...
//   内部使用的方法
  ...
  // 外部使用的方法，跟内部的区别就是alwaysNormalize这个参数不同
...
## Virtual DOM
真正的DOM元素非常庞大，Virtual DOM 就是用一个原生的 JS 对象去描述一个 DOM 节点，所以它比创建一个 DOM 的代价要小很多，vue 使用`VNode` class 描述。
...
```
Vue.js 借鉴了 [snabbdom](https://github.com/snabbdom/snabbdom/tree/master/src/package)
Virtual DOM 除了它的数据结构的定义，映射到真实的 DOM 实际上要经历 VNode 的 create、diff、patch 等过程,那么在 Vue.js 中，VNode 的 create 是通过之前提到的 createElement 方法创建的，我们接下来分析这部分的实现。

## createElement
createElement用于创建 VNode
...
export function createElement (
  ...
  // 核心是这里调用_createElement方法
  return _createElement(context, tag, data, children, normalizationType)
}
```
`createElement`是对`__createElement`的封装
```js
export function _createElement (
  context: Component,
  tag?: string | Class<Component> | Function | Object,
  data?: VNodeData,
  // children表示子节点，后续会被规范为VNode数组
  children?: any,
  // 表示子节点规范的类型，类型不同规范的方法也就不一样，它主要是参考 render 函数是编译生成的还是用户手写的。
...
### children 的规范化
这里主要是调用 `normalizeChildren` 和 `simpleNormalizeChildren` 方法
...
// Vue.js从接口层面允许用户把 children 写成基础类型用来创建单个简单的文本节点，
...
function normalizeArrayChildren (children: any, nestedIndex?: string): Array<VNode> {
  ...
  for (i = 0; i < children.length; i++) {
    ...
    last = res[lastIndex]
    //  nested
    // 处理嵌套，如果child是数组则递归调用
    if (Array.isArray(c)) {
      if (c.length > 0) {
        c = normalizeArrayChildren(c, `${nestedIndex || ''}_${i}`)
        // merge adjacent text nodes
        // 如果上一个child也是text节点则合并
...
### VNode的创建
回到`createElement`函数，规范化`children`后，会创建一个 VNode 的实例
```js
let vnode, ns
if (typeof tag === 'string') {
  let Ctor
...
Vue.prototype.__patch__ = inBrowser ? patch : noop
```
`patch`方法定义在`src/platforms/web/runtime/patch.js`中：
```js
...
```
`createPatchFunction`定义在`src/core/vdom/patch.js`中：
```js
...
export function createPatchFunction (backend) {
  ...
  // ...
  // oldVnode 表示旧的 VNode 节点，可以不存在或者是DOM对象
...
```
在`vm._update`中是这么调用`patch`的：
```js
// initial render
vm.$el = vm.__patch__(vm.$el, vnode, hydrating, false /* removeOnly */)
```
结合我们的例子，我们的场景是首次渲染，所以在执行 patch 函数的时候，传入的`vm.$el`对应的是例子中 id 为 app 的 DOM 对象，这个也就是我们在`index.html`模板中写的`<div>`，`vm.$el`的赋值是在之前 mountComponent 函数做的，vnode 对应的是调用 render 函数的返回值，hydrating 在非服务端渲染情况下为 false，removeOnly 为 false。
